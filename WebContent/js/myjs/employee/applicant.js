 ...
 */
Ext.namespace("hrmsys.employee.applicant")
applicantGridPanel = Ext.extend(Ext.grid.GridPanel,{
	id: 'empInfoapplicant',
	constructor: function(){
		Ext.QuickTips.init();
		applicantStore = new Ext.data.JsonStore({
			url: 'emp_applicantList.action',
			root: 'root',
		...
		applicantGridPanel.superclass.constructor.call(this,{
			viewConfig: {
				forceFit: true
			},
			...
			tbar: new Ext.Toolbar({
				...
				items: ['部门:',new depart("员工"),'&nbsp;&nbsp;&nbsp;条目:',{
					...
					tooltip: '查看员工信息',
					id: 'emp_detail',
					handler: this.applicantAudit
				}]
			  }),
			
		  	bbar: new PagingToolbar(applicantStore, 20)
		});
	...
	empUpdateFn: function(){
		...
		var selectionModel = Ext.getCmp('empInfoapplicant').getSelectionModel();
	...
	applicantAudit: function(){
		var applicantDetailWin = new ApplicantDetailWin();
		var selectionModel = Ext.getCmp('empInfoapplicant').getSelectionModel();
		var record = selectionModel.getSelections();
		if(record.length != 1){
			Ext.Msg.alert('提示','请选择一个');
		...
		var empId = record[0].get('empId');
		Ext.getCmp('applicantDetailId').getForm().load({
			url: 'emp_intoUpdate.action',
			method: 'post',
			params: {
				empId: empId
